...
if (existsSync(configPath)) {
    try {
        ...
        if (Array.isArray(config?.routes)) {
            const existingNoCacheMain = config.routes.some((route) =>
                typeof route?.src === 'string' && route.src.includes('main-app')
            );

            if (!existingNoCacheMain) {
                const immutableIndex = config.routes.findIndex((route) =>
                    typeof route?.src === 'string' && route.src.includes('/_next/static/')
        ...
        console.error(`[pages:prepare] Failed to update config.json cache routes: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
