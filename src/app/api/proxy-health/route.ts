...
export async function GET(request: NextRequest) {
  ...
  const paths = ['/health', '/'];
  const attempts: Array<{ path: string; status?: number; ok: boolean; error?: string }> = [];
  ...
  for (const path of paths) {
    ...
    try {
      ...
      if (response.ok) {
        return NextResponse.json({ ok: true, path, status: response.status, attempts });
      }
    } catch (error: any) {
      attempts.push({ path, ok: false, error: error?.message || 'Network error' });
    }
  }
