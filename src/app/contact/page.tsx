...
export default function ContactPage() {
  ...
  const [threadIdFromQuery, setThreadIdFromQuery] = useState<string | null>(
    null,
  );
  ...
  const [category, setCategory] =
    useState<FeedbackThread["category"]>("feedback");
  ...
  const selectedStatus =
    selectedThreadMeta?.status || selectedThread?.status || "open";
        ...
        <div className="rounded-[24px] border border-white/10 bg-white/[0.02] p-5 sm:p-6 backdrop-blur-xl shadow-[0_10px_28px_rgba(0,0,0,0.35)] flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-muted max-w-lg">
              Report bugs, send feedback, or contact support. Admin replies
              appear here.
            </p>
          </div>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setCategory(
                        event.target.value as FeedbackThread["category"],
                      )
                    }
                  >
                ...
                <button
                  disabled={
                    isSubmitting || (hasTurnstile && !feedbackTurnstileToken)
                  }
                    ...
                    <div className="mt-4 pt-4 border-t border-white/10">
                      ...
                      {isSelectedClosed && (
                        <p className="text-[11px] text-center text-text-muted mt-2">
                          Closed threads are visible for 14 days and cannot be
                          reopened.
                        </p>
                      )}
                    </div>
