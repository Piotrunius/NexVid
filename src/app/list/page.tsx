...
export default function WatchlistPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden pb-10 pt-24">
      <div className="px-4 sm:px-6 lg:px-10 xl:px-14 2xl:px-16">
        <div className="mb-5 rounded-[24px] border border-white/10 bg-white/[0.02] p-5 shadow-[0_10px_28px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:p-6">
          <h1 className="text-[30px] font-bold tracking-tight text-text-primary">My List</h1>
        ...
        <div className="mb-7 mt-2 flex items-center justify-between gap-4">
          <h2 className="text-[20px] font-semibold tracking-tight text-text-primary">Watchlist</h2>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as typeof sortBy)}
            className="input !w-auto min-w-[170px] !px-3 !py-2 !text-[12px]"
          >
        ...
        <div className="custom-scrollbar -mx-4 mb-2 w-full overflow-x-auto px-4 pb-4 sm:mx-0 sm:px-0">
          <div className="flex w-max gap-2 rounded-full bg-white/5 p-1">
            <button
              onClick={() => setActiveStatus('all')}
              className={`flex shrink-0 items-center gap-2 whitespace-nowrap rounded-full border px-4 py-1.5 text-[11px] font-black uppercase tracking-wider transition-all ${activeStatus === 'all' ? 'border-accent-glow bg-accent-muted text-accent' : 'border-transparent bg-transparent text-white/40 hover:text-white'}`}
            >
            ...
            {STATUSES.map((s) => (
              <button
                ...
                className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border px-4 py-1.5 text-[11px] font-black uppercase tracking-wider transition-all ${activeStatus === s.key ? 'border-accent-glow bg-accent-muted text-accent' : 'border-transparent bg-transparent text-white/40 hover:text-white'}`}
              >
        ...
        {continueWatching.length > 0 && (
          <section className="mb-8">
            <h2 className="mb-3 text-[15px] font-semibold text-text-primary">Continue Watching</h2>
        ...
        {filteredItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--bg-glass-light)]">
              <svg
            ...
            <p className="mt-1 text-[13px] text-text-muted">Start browsing to add movies & shows</p>
  ...
  return (
    <div className="glass-card glass-liquid group relative">
      <button
        ...
        className="bg-[var(--bg-primary)]/75 absolute right-2 top-2 z-10 rounded-[8px] p-1.5 text-text-secondary opacity-100 backdrop-blur-sm transition-all hover:text-red-400"
        title="Remove"
      >
        <svg
          width="14"
      ...
      <Link href={href} className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1 py-1">
          ...
          <div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--bg-tertiary)]">
            <div
              className="h-full rounded-full bg-accent shadow-[0_0_6px_var(--accent-glow)]"
  ...
  return (
    <div className={cn('glass-card group relative', isActive && 'z-40 overflow-visible')}>
      <Link href={link} className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1 py-1">
          <p className="truncate text-[13px] font-semibold text-text-primary">{item.title}</p>
          <p className="mt-0.5 text-[11px] capitalize text-text-muted">{item.mediaType}</p>
          ...
          {item.progress && item.progress.percentage != null && (
            <div className="mt-2">
              <div className="h-1 overflow-hidden rounded-full bg-[var(--bg-tertiary)]">
                <div
                  className="h-full rounded-full bg-accent shadow-[0_0_6px_var(--accent-glow)] transition-all"
      ...
      <div
        className={cn(
          'watchlist-menu-container absolute right-2 top-2 opacity-100 transition-opacity',
        )}
      >
        <div className="relative">
          <button
            ...
            className="bg-[var(--bg-primary)]/80 rounded-[8px] p-1.5 text-text-secondary backdrop-blur-sm hover:text-text-primary"
          >
          ...
          {isActive && (
            <div
              className={cn(
                'panel-glass absolute right-0 z-50 w-40 origin-top-right animate-scale-in rounded-[12px] p-1.5',
            ...
            >
              {STATUSES.map((s) => (
                <button
                  ...
                  className={cn(
                    'flex w-full items-center gap-2 rounded-[8px] px-3 py-1.5 text-left text-[12px] transition-colors',
                    item.status === s.key
              ...
              <button
                ...
                className="flex w-full items-center gap-2 rounded-[8px] px-3 py-1.5 text-left text-[12px] text-red-400 transition-colors hover:bg-red-500/10"
              >
                <svg
                  width="12"
                  height="12"
