...
export function AdminSurveys({ canDelete }: { canDelete: boolean }) {
  ...
  const visibleSurveys = useMemo(() => {
    return sortedSurveys.filter((survey) =>
      showArchived ? survey.is_archived === 1 : survey.is_archived !== 1,
    );
  ...
  const selectResults = async (id: string) => {
    ...
    setResponses([]);
    try {
      const res = await cloudFetch<{ responses: SurveyResponse[] }>(
        `/admin/surveys/results?id=${id}`,
      );
          ...
          {viewingResults && (
            <div className="glass-card p-4 animate-slide-up border-white/10 shadow-[0_24px_48px_rgba(0,0,0,0.4)]">
              ...
              <div className="space-y-4">
                {Object.entries(getAnalytics(viewingResults) || {}).map(
                  ([id, stat]: [string, any]) => (
                    <div
                      key={id}
                    ...
                    <div className="space-y-1.5 pl-2 border-l border-white/10 mt-1">
                      {q.options?.map((opt, oIdx) => (
                        <div key={oIdx} className="flex gap-2">
                          <input
                            className="input-minimal flex-1 text-[11px]"
