              ...
              <>
                ...
                {notifications.length === 0 && announcements.length === 0 ? (
                  ...
                  <div className="max-h-80 overflow-y-auto p-2 space-y-1">
                    ...
                    {announcements.map((a) => (
                      <div key={a.id} className="flex items-start gap-2.5 rounded-[14px] px-3 py-2.5 hover:bg-white/[0.06] transition-colors duration-300">
                        ...
                        <button onClick={() => { dismissId(a.id); setAnnouncements((prev) => prev.filter((x) => x.id !== a.id)); }} className="shrink-0 rounded-[10px] p-1 hover:bg-white/[0.06] text-white/20 hover:text-white/60 transition-colors duration-300">
          ...
          {isLoggedIn ? (
            <div className="relative">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-accent to-accent-hover text-[12px] font-bold text-white shadow-[0_0_20px_var(--accent-glow)] transition-all duration-500 hover:scale-110 hover:shadow-[0_0_30px_var(--accent-glow)] active:scale-95 sm:h-11 sm:w-11 sm:text-[13px]"
              ...
              {isProfileOpen && (
                <div className="panel-glass absolute right-0 top-[calc(100%+8px)] z-50 w-56 overflow-hidden p-2 animate-scale-in">
                  ...
                  {user?.isAdmin && (
                    <Link href="/admin" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-2.5 rounded-[12px] px-3 py-2.5 text-[13px] text-white/50 hover:bg-white/[0.06] hover:text-white transition-all duration-300">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                      Admin Panel
                    </Link>
                  )}
                  {isLoggedIn && (
                    <Link href="/contact" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-2.5 rounded-[12px] px-3 py-2.5 text-[13px] text-white/50 hover:bg-white/[0.06] hover:text-white transition-all duration-300">
                  ...
                  <button
                    onClick={() => setShowProfilePartyInput((value) => !value)}
                    className="flex w-full items-center gap-2.5 rounded-[12px] px-3 py-2.5 text-[13px] text-white/50 hover:bg-white/[0.06] hover:text-white transition-all duration-300"
                  >
                  ...
                  {showProfilePartyInput && (
                    <div className="mb-1 mt-1 space-y-2 rounded-[12px] bg-white/[0.04] p-2">
                      <input
                        value={partyCode}
                        onChange={(e) => setPartyCode(e.target.value.toUpperCase())}
                        placeholder="Room code"
                        className="w-full rounded-[10px] bg-white/[0.08] px-3 py-2 text-[12px] text-white placeholder:text-white/35 outline-none"
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            handleWatchPartyJoin();
                          }
                        }}
                      />
                      <button
                        ...
                        className="w-full rounded-[10px] bg-accent px-3 py-2 text-[12px] font-semibold text-white disabled:opacity-50"
                      >
                        Join Room
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-2.5 rounded-[12px] px-3 py-2.5 text-[13px] text-red-400/80 hover:bg-red-500/10 hover:text-red-400 transition-all duration-300"
                  >
            ...
            >
              <svg className="sm:hidden" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
