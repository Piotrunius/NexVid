...
export default function ExternalRatings({
  ...
  const fetchRatings = async () => {
    ...
    try {
      ...
      if (data.Response === "True") {
        ...
        const hasRatingsArray = Boolean(
          data.Ratings && data.Ratings.length > 0,
        );
        const hasMetascore = Boolean(
          data.Metascore && data.Metascore !== "N/A",
        );

        const otherRatings =
          data.Ratings?.filter(
            (r) => !r.Source.toLowerCase().includes("internet movie"),
          ) || [];
  ...
  const hasMetacriticInArray = ratings?.some((r) =>
  ...
  return (
    <div
      className={cn(
        ...
        vertical ? "flex-col items-end" : "flex-wrap",
      )}
    >
