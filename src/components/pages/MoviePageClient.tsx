            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              <div className="relative">
                <button onClick={() => setShowWatchlistMenu(v => !v)} className="btn-glass !px-4 !py-3">
                  ...
                  {watchlistItem && watchlistItem.status !== 'none' ? watchlistItem.status : 'Add to List'}
                </button>
                {showWatchlistMenu && (
              ...
              {/* SeriesGraph Link */}
              <a
                href={`https://seriesgraph.com/movie/${movie.tmdbId || movie.id}-${movie.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}`}
                target="_blank"
                rel="noopener noreferrer"
