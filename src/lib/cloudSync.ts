...
async function fetchWithTimeout(
  ...
  const timeoutId = setTimeout(
    () => controller.abort(new Error(`Cloud API request timed out after ${timeoutMs}ms`)),
    timeoutMs,
  );
...
export async function deleteAdminUserByUsername(identifier: string) {
  return cloudFetch(`/admin/users?identifier=${encodeURIComponent(identifier)}`, {
    method: 'DELETE',
  });
}
...
export async function revokeAdminPermission(userId: string) {
  return cloudFetch<{ ok: boolean }>(`/admin/grant?userId=${encodeURIComponent(userId)}`, {
    method: 'DELETE',
  });
}

export async function clearAllActiveSessions() {
  return cloudFetch<{ ok: boolean; clearedCount: number }>('/admin/sessions/clear', {
    method: 'POST',
  });
}
