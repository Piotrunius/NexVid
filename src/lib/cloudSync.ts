...
async function fetchWithTimeout(
  ...
  const timeoutId = setTimeout(
    () =>
      controller.abort(
        new Error(`Cloud API request timed out after ${timeoutMs}ms`),
      ),
    timeoutMs,
  );
...
export async function cloudFetch<T = any>(
  ...
  // Persistent anonymous identifier for occupancy stats (no IP/Device tracking)
  if (typeof window !== "undefined") {
    let clientId = localStorage.getItem("nexvid-client-id");
    if (!clientId) {
      clientId = crypto.randomUUID();
      localStorage.setItem("nexvid-client-id", clientId);
    }
    headers.set("NexVid-Client-ID", clientId);
  }
...
export async function loadCloudSettings() {
  return cloudFetch<{ settings: Record<string, unknown> }>("/user/settings", {
    method: "GET",
  });
}
...
export async function changeCloudPassword(payload: {
...
export async function setAdminAccountLimit(
  type: "username",
  value: string,
  maxAccounts: number,
) {
...
export async function deleteAdminAccountLimit(type: "username", value: string) {
  return cloudFetch(
    `/admin/account-limits?type=${encodeURIComponent(type)}&value=${encodeURIComponent(value)}`,
    { method: "DELETE" },
  );
}
...
export async function banAdminTarget(
  type: "username" | "ip",
  value: string,
  reason: string,
) {
...
export async function unbanAdminTarget(type: "username" | "ip", value: string) {
  ...
  return cloudFetch(`/admin/bans?type=${encodedType}&value=${encodedValue}`, {
    method: "DELETE",
  });
}

export async function loadAdminAnnouncements() {
  return cloudFetch<{ items: any[] }>("/admin/announcements", {
    method: "GET",
  });
}
...
export async function loadAdminBlockedMedia() {
  ...
  }>("/admin/blocked-media", { method: "GET" });
}

export async function addAdminBlockedMedia(
  tmdbId: string,
  mediaType: "movie" | "tv",
  reason?: string,
) {
...
export async function deleteAdminBlockedMedia(
  tmdbId: string,
  mediaType: "movie" | "tv",
) {
  return cloudFetch(
    `/admin/blocked-media?tmdbId=${encodeURIComponent(tmdbId)}&mediaType=${encodeURIComponent(mediaType)}`,
    { method: "DELETE" },
  );
}

export async function loadPublicBlockedMedia() {
  return cloudFetch<{ items: { tmdbId: string; mediaType: string }[] }>(
    "/public/blocked-media",
    {
      method: "GET",
      cache: "no-store",
    },
...
export async function deleteAdminAnnouncement(id: string) {
  return cloudFetch(`/admin/announcements?id=${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
...
export async function deleteAdminUserByUsername(identifier: string) {
  return cloudFetch(
    `/admin/users?identifier=${encodeURIComponent(identifier)}`,
    { method: "DELETE" },
  );
}

export async function resetUserPassword(identifier: string) {
  return cloudFetch<{ ok: boolean; temporaryPassword?: string }>(
...
export async function revokeAdminPermission(userId: string) {
  return cloudFetch<{ ok: boolean }>(
    `/admin/grant?userId=${encodeURIComponent(userId)}`,
    { method: "DELETE" },
  );
}

export async function clearAllActiveSessions() {
  return cloudFetch<{ ok: boolean; clearedCount: number }>(
...
export async function loadAdminAuditLogs(params?: {
  ...
  return cloudFetch<{
    ...
    `/admin/audit-logs?limit=${encodeURIComponent(String(limit))}&offset=${encodeURIComponent(String(offset))}`,
    { method: "GET" },
  );
}
...
export async function reportPlayerError(
  mediaType: string,
  mediaId: string,
  code: string,
  message: string,
  isFebboxAuth: boolean,
  febboxToken?: string,
) {
  return { ok: true };
}
...
export async function loadAdminFebboxTokens() {
  ...
  }>("/admin/febbox-tokens", { method: "GET" });
}
...
export async function updateAdminFebboxToken(
  token: string,
  isActive?: boolean,
  isBanned?: boolean,
) {
...
export async function deleteAdminFebboxToken(token: string) {
  return cloudFetch(`/admin/febbox-tokens?token=${encodeURIComponent(token)}`, {
    method: "DELETE",
  });
}
...
export async function createUserFeedbackThread(
  payload: {
    category: "bug" | "feedback" | "contact" | "feature";
    subject: string;
    message: string;
...
export async function sendUserFeedbackMessage(
  threadId: string,
  message: string,
) {
...
export async function loadAdminFeedbackMessages(threadId: string) {
  ...
  }>(`/admin/feedback/messages?threadId=${encodeURIComponent(threadId)}`, {
    method: "GET",
  });
}

export async function replyAdminFeedbackThread(payload: {
  threadId: string;
  message: string;
  status?: "open" | "answered" | "closed";
}) {
...
export async function deleteAdminFeedbackThread(threadId: string) {
  return cloudFetch<{ ok: boolean; deletedThreadId: string }>(
    `/admin/feedback/thread?threadId=${encodeURIComponent(threadId)}`,
    {
      method: "DELETE",
    },
  );
...
export async function leaveWatchParty(payload: {
  roomId: string;
  participantId: string;
}) {
  return cloudFetch<{
