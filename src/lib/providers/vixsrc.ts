...
export class VixSrcProvider extends BaseProvider {
    ...
    private async getSources(
        ...
        try {
            ...
            // Build master playlist URL with preferred language (always English)
    ...
    private buildMasterUrl(
        ...
        return `${playlist}${separator}token=${token}&expires=${expires}&h=1&lang=${preferredLang}`;
    }
    ...
    private parseAudioTracks(
        ...
        for (const line of lines) {
            ...
            // DEBUG: log found audio tracks so we can see non-standard language codes
            console.log(`VixSrc: Found track: ${label} [${language}]`);
