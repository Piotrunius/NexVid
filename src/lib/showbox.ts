...
export async function getFebBoxToken(clientId: string, clientSecret: string): Promise<FebBoxTokenResponse> {
  try {
    ...
    if (!res.ok || !ct.includes('json')) {
      return { code: 0, msg: `FebBox OAuth: HTTP ${res.status} (${ct.split(';')[0] || 'non-json'})` };
    }
...
export async function febboxGetLinks(
  ...
  fid: number,
  uiCookie?: string,
): Promise<{ qualities: FebBoxQuality[]; subtitles: FebBoxSubtitle[]; audioTracks: FebBoxAudioTrack[] }> {
  ...
  const parseDirectSubtitles = (directData: any): FebBoxSubtitle[] => {
    const fileEntry = Array.isArray(directData?.data) && directData.data.length > 0 ? directData.data[0] : {};
    ...
    const mapped = items
      .map((item) => {
        if (typeof item === 'string') {
          ...
          return { url: item, language: guessLang(item), label: guessLang(item).toUpperCase(), type } as FebBoxSubtitle;
        }

        const url = item?.url || item?.src || item?.download_url || item?.file_path || '';
        if (!url) return null;

        const language = item?.language || item?.lang || item?.locale || guessLang(String(item?.label || item?.name || url));
        const label = item?.label || item?.name || String(language).toUpperCase();
    ...
    const mapped = items
      .map((item, index) => {
        if (typeof item === 'string') {
          ...
          return {
            id: index,
            name: lang !== 'unknown' ? lang.toUpperCase() : `Track ${index + 1}`,
            lang,
            isDefault: index === 0,
            url: item,
          } as FebBoxAudioTrack;
        ...
        const url = item?.url || item?.src || item?.download_url || item?.file_path || undefined;
        const lang = String(
          item?.language || item?.lang || item?.locale || item?.code || guessLang(item?.name || item?.label || ''),
        ).toLowerCase();
        const name = String(item?.label || item?.name || item?.title || (lang !== 'unknown' ? lang.toUpperCase() : `Track ${index + 1}`));
        ...
        return {
          ...
          name,
          lang,
          isDefault: Boolean(item?.default || item?.is_default || item?.enabled || index === 0),
          url,
        } as FebBoxAudioTrack;
      })
  ...
  const parseDirectLinks = (directData: any): { qualities: FebBoxQuality[]; subtitles: FebBoxSubtitle[]; audioTracks: FebBoxAudioTrack[] } => {
    if (!(directData?.code === 1 && Array.isArray(directData?.data) && directData.data.length > 0)) {
      return { qualities: [], subtitles: [], audioTracks: [] };
    }

    const fileEntry = directData.data[0] || {};
    const qualityList = Array.isArray(fileEntry.quality_list) ? fileEntry.quality_list : [];
...
export interface ResolvedStream {
  ...
  audioTracks: { id: number; name: string; lang: string; isDefault: boolean; url?: string }[];
  shareKey: string;
  fileName?: string;
}
...
export async function resolveStream(options: {
  ...
  logs.push({ step: 'match', status: 'ok', detail: `id=${showboxId} "${bestMatch.title || bestMatch.name}"` });
  ...
  try {
    ...
    if (!shareKey) {
      logs.push({ step: 'shareKey', status: 'fail', detail: 'All share-link hosts failed' });
      return { stream: null, logs };
    }
  ...
  try {
    ...
    if (!files || files.length === 0) {
      logs.push({ step: 'fileList', status: 'fail', detail: 'Empty file list' });
      return { stream: null, logs };
    }
  ...
  try {
    if (type === 'movie') {
      ...
      if (seasonDir) {
        ...
        logs.push({ step: 'seasonNav', status: 'ok', detail: seasonDir.file_name });
      }
    ...
    if (!targetFile) {
      logs.push({ step: 'findFile', status: 'fail', detail: 'No target file found' });
      return { stream: null, logs };
    }
    logs.push({ step: 'findFile', status: 'ok', detail: `${targetFile.file_name} (fid=${targetFile.fid})` });
  } catch (err: any) {
  ...
  try {
    ...
    logs.push({ step: 'getLinks', status: 'ok', detail: `${links.length} qualities, ${linkData.subtitles.length} subtitles, ${linkData.audioTracks.length} audio tracks` });
